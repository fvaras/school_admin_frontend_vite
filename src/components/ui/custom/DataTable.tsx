...
interface DataTableProps<T> {
  data: T[]
  columns: ColumnDef<T, ColumnMeta>[]
}
...
const formatCellValue = (value: any, format?: string) => {
  if (format === "currency") {
    ...
    const formattedValue = new Date(value).toLocaleTimeString()
    return <div className="text-right">{formattedValue}</div>
  } else {
              ...
              table.getRowModel().rows.map((row) => (
                <TableRow key={row.id} data-state={row.getIsSelected() && "selected"}>
                  {row.getVisibleCells().map((cell) => {
                    // @ts-ignore
                    const formattedValue = formatCellValue(row.getValue(cell.column.id), cell.column.columnDef.meta?.format)
                    return (
